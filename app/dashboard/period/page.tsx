...
export default function CycleTrackerPage() {
  ...
  async function loadData() {
    try {
      ...
      hist.forEach((p: any) => {
        ...
        p.days?.forEach((d: any) => flowMap.set(toLocalDateKey(d.date), d.flow));
      });

      setWeekView(last7.map(d => ({
        ...d,
      ...
      setHistory(hist);
      setPrediction(pred);
      setIsActivePeriod(!!hist.find((p: any) => !p.endDate));
    } catch (err) {
  ...
  useEffect(() => {
    if (!prediction?.nextExpectedDate) setAuraInsight("Log more cycles to unlock predictions");
    else if (prediction.confidence < 50) setAuraInsight("Your cycle is learning — keep logging");
    else setAuraInsight("Your cycle rhythm is becoming clear");
  }, [prediction]);
  ...
  const handleSave = async () => {
    ...
    setIsSaved(true);
    try {
      if (!isActivePeriod) await startPeriod(new Date(), selectedFlow);
      else await logFlowDay(new Date(), selectedFlow);
      await loadData();
      setTimeout(() => { setIsLogging(false); setIsSaved(false); }, 1200);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8 pt-24">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        {/* 1. Aura Insight (Mirrors AI Analysis style) */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="bg-primary/5 border-primary/20 backdrop-blur-sm p-5 rounded-2xl">
            <div className="flex items-center gap-2 text-primary mb-2">
              <Sparkles className="w-4 h-4" />
              <span className="text-xs font-bold uppercase tracking-widest text-primary">Aura Insight</span>
            </div>
              ...
              {isLogging && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                  <div className="mt-4 p-6 bg-muted/30 border border-border rounded-2xl space-y-8">
                    ...
                    <Button className="w-full h-12 rounded-full font-bold bg-slate-900 text-white shadow-lg" onClick={handleSave} disabled={isSaved || !selectedFlow}>
          ...
          <div className="flex justify-between md:justify-center gap-2 md:gap-6">
            {weekView.map((d, i) => (
              <div key={i} className="flex flex-col items-center gap-2">
                ...
                <div className={`w-10 h-14 rounded-2xl flex items-center justify-center text-sm font-bold transition-all border ${d.current ? 'ring-2 ring-rose-500 ring-offset-2 scale-110 shadow-md' : ''} ${!d.flow ? 'bg-muted/30 border-transparent' : 'bg-rose-400 text-white border-rose-500'}`}>
                  {d.date}
                </div>
              </div>
            ))}
        ...
        <Card className="p-8 rounded-3xl overflow-hidden bg-card border-border shadow-sm">
          ...
          <div className="flex flex-row items-center gap-10">
            ...
            <div className="flex flex-col gap-4 flex-1">
              <div className="p-5 bg-muted/40 rounded-2xl border border-border/50 min-h-[80px] flex flex-col justify-center">
        ...
        {/* 5. Cycle History - Revamped Folder Card */}
                          ...
                          <div className="flex items-end justify-between px-2">
                            <div className="flex flex-col">
                              <div className="flex items-center gap-1.5 mb-2 text-muted-foreground">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-[0.15em]">Month</span>
                              </div>
                              ...
                              <div className={`${fraunces.className} text-3xl font-black text-[#2d3436] flex items-start`}>
                                {p.endDate ? (
                                  <>
                                    ...
                                    <span className="text-sm ml-0.5 mt-1 font-bold">{getOrdinal(new Date(p.endDate).getDate())}</span>
                                  </>
